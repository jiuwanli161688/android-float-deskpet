...
第一次打开先看几页引导，再留下账号（只在本机），然后选陪伴、起名字。登录后进入「今日陪伴」。女角色风格：御姐 / 萝莉 / 清纯风；男角色：温柔邻家 / 清爽少年 / 沉稳学长 / 大叔 / 成功人士。不取名时会从名字册里选一个。可拖动、点一下会跳/眨眼，待机会走路、打盹、躲到屏幕边上探头。打开穿透后触摸会穿过桌宠，方便点底下的微信或浏览器。
...
- Debug：`releases/float-deskpet-1.3.4-debug.apk`
- Release（debug 签名，可直接装）：`releases/float-deskpet-1.3.4-release.apk`
...
女风格独立造型：`pet_f_yujie_*` / `pet_f_luoli_*` / `pet_f_qingchun_*`。男风格：`pet_m_wenrou_*` / `pet_m_qingshuang_*` / `pet_m_chenwen_*` / `pet_m_dashu_*` / `pet_m_chenggong_*`。

同名预览也在 `art/character/`。

## 版本

见 `releases/CHANGELOG.md`。当前 `1.3.4`。
